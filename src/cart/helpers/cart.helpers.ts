...
export function calculateDiscountedPrice(product: {
  ...
  const discountPercent = parseDiscountPercent(product.discount);

  if (discountPercent === null || discountPercent === 0) {
    return price;
  }

  return roundMoney(price * ((100 - discountPercent) / 100));
}
...
function parseDiscountPercent(
  ...
  const parsedDiscount =
    typeof discount === 'number'
      ? discount
      : Number.parseFloat(discount.replace(',', '.').replace(/[^\d.]/g, ''));
